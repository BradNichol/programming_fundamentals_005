...
*/
function min(object) {
  // Object.values returns the values from object into an array
  const arr = Object.values(object);
  // Math.min can't be used on array so use..., a destructuring assignment that unpacks data from array into distinct variables.
  return Math.min(...arr);
}
...
function invert(object) {
  ...
  // get the key / values in arrays
  const keyArray = Object.keys(object);
  const valueArray = Object.values(object);
  
  // loop over array and set key as value
...
function addAll(arr) {
  ...
  // loop over each object in array and get key / values as arrays
  arr.forEach((obj) => {
...
function find(obj, matcherFunc) {
  for (let k in obj) {
    // pass object value into mfunc. If returns true, return that object value.
    if (matcherFunc(obj[k])) {
      return obj[k];
    }
    
  }
  // if above doesn't return true, return null.
  return null;
}
